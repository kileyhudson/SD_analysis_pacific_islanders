# violin_pi_filtering_comparison2.R
# Plot PI SD pairs across different filtering conditions
#No HPRC
...
before <- structural %>%
  select(Sample, Haplotype, SD_pairs_before_filtering) %>%
  mutate(Condition = "unfiltered",
         SD_pairs = SD_pairs_before_filtering) %>%
  select(-SD_pairs_before_filtering)

struct_only <- structural %>%
  select(Sample, Haplotype, SD_pairs_after_filtering) %>%
  mutate(Condition = "structural error filter",
         SD_pairs = SD_pairs_after_filtering) %>%
  select(-SD_pairs_after_filtering)
...
df <- bind_rows(before, struct_only, both) %>%
  ...
  mutate(Condition = factor(Condition, levels = c("unfiltered", "structural error filter", "all errors filter")))
...
# plot -----------------------------------------------------------------
p <- ggplot(df, aes(x = Condition, y = SD_pairs)) +
  ...
  labs(title = "SD pairs across different filtering conditions",
       x=NULL, y = "Number of SD pairs") +
  theme_classic(base_size = 15)

# save -----------------------------------------------------------------
ggsave("violin_error_filtering_comparison2.png", p, width = 7.5, height = 5, dpi = 300)
